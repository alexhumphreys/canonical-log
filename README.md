...
For an entry point Spring doesn't instrument for you, open the unit explicitly with `withCanonicalLogBlocking(adapter, input, emit) { ... }` (or `withCanonicalLog` for suspend code) and a hand-written `WorkUnitAdapter` — that's all the scheduling starter does internally. When the lifecycle boundaries arrive as *separate callbacks* — a message-consumer listener that receives on one thread and acks/nacks on another — use the public `openCanonicalWorkUnit` / `CanonicalWorkUnitScope` open/close pair instead. **[docs/recipes/message-consumers.md](docs/recipes/message-consumers.md)** is the worked, broker-agnostic recipe for message consumers (queue poll loops, split-callback listeners, DLQ marking, suspend hand-off), compiled and pinned by `MessageConsumerRecipeTest`.
...
- `canonical-log-jobrunr` — `CanonicalJobServerFilter` for JobRunr: a `background_job` line per processing attempt with `job_id`/`job_name`/`job_attempt` (JobRunr is `compileOnly`; your own OSS/Pro version wins at runtime)
