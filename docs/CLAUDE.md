...
- **Plain thread hops need the propagation helpers.** The coroutine bridge doesn't cover `ExecutorService`/`@Async`/`CompletableFuture.supplyAsync` — the worker thread has no binding, so contributions are silent no-ops. `ContextPropagation.kt` provides `Runnable`/`Callable`/`Executor.propagatingCanonicalContext()` (capture at wrap/submit time on the submitting thread, bind/restore around the task). The work unit still does not *wait* for such tasks — join before the block returns or contributions are cut off at emit. Notably this cannot fix OkHttp `enqueue()` (queued calls are promoted from OkHttp-owned threads); documented on `OkHttpCanonicalInterceptor`. Pinned by `ContextPropagationTest`.
