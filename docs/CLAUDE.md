...
- `canonical-log-core` — accumulator, `WorkUnit`, `WorkUnitAdapter`, `Outcome`, `CanonicalLogContext`, the `ThreadContextElement` bridge, the `CanonicalLogMdc` MDC mirror (see the gotchas entry), `CanonicalFields` (the published `const` names for every field the library writes — see the field-constants gotcha), and the `openCanonicalWorkUnit`/`CanonicalWorkUnitScope` blocking open/close primitive (delicate; see the open/close-primitive gotcha). Framework-agnostic.
