...
- `canonical-log-core` — accumulator, `WorkUnit`, `WorkUnitAdapter`, `Outcome`, `CanonicalLogContext`, the `ThreadContextElement` bridge, the `CanonicalLogMdc` MDC mirror (see the gotchas entry), and `CanonicalFields` (the published `const` names for every field the library writes — see the field-constants gotcha). Framework-agnostic.
